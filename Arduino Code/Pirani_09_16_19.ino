...
typedef struct Calibration
{
  ...
  float b[gauges];
  float c[gauges];
  unsigned int v[gauges];
  unsigned int w[gauges];
...
//  gainset=.1875/1000;  // 2/3x gain +/- 6.144V  1 bit = 3mV      0.1875mV (default)
  ads.begin();
  lcd.begin(20,4);
...
void IEEE(){
   String req = inputString.substring(1,5);
   if (req=="IDN?"){
    Serial.println(IDN);
   }
   else if (req=="CFG?"){
...
//byte sizes[8] = {4,4,4,2,2,5,1,1}; //Size in the EEPROM of each configuration value
//int Addr[sizeof(sizes)][4]; //Matrix that contains all address values
